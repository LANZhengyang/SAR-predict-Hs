...
data_loader_1516 = torch.utils.data.DataLoader(dataset=dataset_1516, batch_size=512,shuffle=True,num_workers=2)

dataset_2017 = Dataset_torch(data_17)
data_loader_2017 = torch.utils.data.DataLoader(dataset=dataset_2017, batch_size=512,num_workers=2)
...
class Net_comb(pl.LightningModule):
    ...
    def forward(self, x1,x2):
        ...
        x = self.dropout_n2(x)
        output = F.softplus(self.fc256_1(x))
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = F.mse_loss(y_pre[:,0], torch.squeeze(y_true).float())        # loss = F.mse_loss(torch.squeeze(y_pre), torch.squeeze(y_true).float())
    ...
    def validation_step(self, batch, batch_idx):
        ...
        loss = F.mse_loss(y_pre[:,0], torch.squeeze(y_true))        # loss = F.mse_loss( torch.squeeze(y_pre), torch.squeeze(y_true).float())
...
checkpoint_callback = ModelCheckpoint(
    monitor='val_loss',
    dirpath='./model/models_orignal_mean_batch_512_p10',
    filename='model-DNN-{epoch:02d}-{val_loss:.4f}',
    save_top_k=1,
    mode='min',
)
